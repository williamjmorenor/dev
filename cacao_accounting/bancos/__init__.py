...
def bancos_pago(payment_id):
    ...
    referencias = (
        database.session.execute(database.select(PaymentReference).filter_by(payment_id=payment_id)).scalars().all()
    )

    # Nombres para mostrar
    banco = database.session.get(BankAccount, registro.bank_account_id) if registro.bank_account_id else None
    banco_destino = (
        database.session.get(BankAccount, registro.target_bank_account_id) if registro.target_bank_account_id else None
    )
